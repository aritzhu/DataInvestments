...
export const analyticsAPI = {
  async getOverview(): Promise<{
    ...
    const res = await fetch(`${API_BASE}/api/analytics/overview`, { headers: authHeaders() });
    if (!res.ok) throw new Error('Failed to fetch analytics');
    return res.json();
  },

  async getTopEvents(days = 7): Promise<{ event: string; count: number }[]> {
    const res = await fetch(`${API_BASE}/api/analytics/top-events?days=${days}`, { headers: authHeaders() });
    if (!res.ok) throw new Error('Failed to fetch top events');
    return res.json();
  },

  async getDaily(days = 30): Promise<{ date: string; count: number }[]> {
    const res = await fetch(`${API_BASE}/api/analytics/daily?days=${days}`, { headers: authHeaders() });
